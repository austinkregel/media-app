...
module.exports = class FileController {
	async index(req, res) {
		...
		let results = await Media.query().eager('[genres, files]').range(page * limit - limit, limit * page);
...
`,
					[ query, likeRightQuery, likeLeftQuery, likeInbetweenQuery, likeAllQuery ]
				)
			)
			.eager('genres', 'file')
			.where('name', '=', query)
	...
	async show(req, res) {
		return await Media.query().eager('files').findById(req.params.id);
	}
	...
	async showFiles(req, res) {
		const files = await File.query()
			...
			.orWhere('hash', '=', req.params.id);

		return files[0];
	}

	async update(req, res) {
		const file = await Meida.query().findById(req.params.id);
