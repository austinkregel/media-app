			...
			.reduce(
				(files, file) =>
					fs.statSync(path.join(dir, file)).isDirectory()
						? files.concat(app.fs.find_recursive(path.join(dir, file)))
...
app.slugify = ($title = '', $separator = '-') => {
	...
	$title = $title
		...
		.replace(new RegExp('/![^' + $separator + 'pLpNs]+/'), '')
		.replace('/[^\x20-\x7E]/u', '')
		.trim();
...
app.searchify = (query) =>
	app.slugify(query).replace(/[-|_]+/g, ' ').split(' ').filter((i) => i.trim()).join(' ').trim();
