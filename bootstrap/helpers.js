...
app.slugify = ($title = '', $separator = '-') => {
    for (let $key in charsArray) {
        let $val = charsArray[$key];
        $title = $title.replace($val, $key);
    }
    // Convert all dashes/underscores into separator
    let $flip = $separator === '-' ? '_' : '-';

    $title = $title.replace('/[^\x20-\x7E]/u', '', )
        .replace($flip, $separator)
        .replace(new RegExp('/![\\' + $flip + ']+/!u'), '-')
        .replace('@', $separator+'at-')
        .replace(new RegExp('/![^'+$separator+'\pL\pN\s]+/'), '')
        .replace('/[^\x20-\x7E]/u', '')
        .trim()

    const splits = $title.split('')

    return splits.map(s => newMappings[s] || s).join('')
        .replace(new RegExp('/![\s]+/'), $separator)
        .replace(/^-+|-+$/g,'');}

app.searchify = (query) => app.slugify(query).replace(/[-|_]+/g, ' ').split(' ').filter(i => i.trim()).join(' ').trim()